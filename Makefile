...
clean:
	multipass exec $(ROCK_DEV) -- bash -c 'docker ps -q -f "name=$(DOCKER_NAME)" && docker stop $(DOCKER_NAME) && docker rm $(DOCKER_NAME) && docker rmi charmed-trino-rock:$(ROCK_VERSION)'
	multipass exec $(ROCK_DEV) -- bash -c 'rm $(REPO_FOLDER)/charmed-trino-rock_$(ROCK_VERSION)-$(UBUNTU_VER)-edge_amd64.rock'
...
run:
	multipass exec $(ROCK_DEV) -- bash -c 'cd $(REPO_FOLDER) &&\
		sudo skopeo --insecure-policy copy oci-archive:charmed-trino-rock_$(ROCK_VERSION)-$(UBUNTU_VER)-edge_amd64.rock docker-daemon:charmed-trino-rock:$(ROCK_VERSION)'
	multipass exec $(ROCK_DEV) -- bash -c "cd $(REPO_FOLDER) &&\
                docker run -d --name $(DOCKER_NAME) -p $(DOCKER_PORT):$(DOCKER_PORT) charmed-trino-rock:$(ROCK_VERSION) $(DOCKER_ARGS)"
	@IP_ADDRESS=$$(multipass --format=table list | grep $(ROCK_DEV) | awk '{print $$3}') && \
	echo "Waiting for Trino..."; \
	while ! nc -z $$IP_ADDRESS $(DOCKER_PORT); do \
		sleep 1; \
	done && \
	echo "Trino is up on $$IP_ADDRESS:$(DOCKER_PORT)"
